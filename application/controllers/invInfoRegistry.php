<?php
	...
	class InvInfoRegistry extends MY_Controller {	
		...
		$sqlString = "SELECT * FROM %s ORDER BY status DESC,%s ASC";
		...
		//查詢倉庫工具資料並回傳
		public function getInventoryTool($id,$type='0'){
			//取得服務資料
			$infoData = array();
			$table = $this->tableName['linkTable']['item'];
			$tableID = $table."ID";
			$sqlString = "SELECT * FROM %s WHERE warehouseID='%s'";					
			$subString = "";
			$handleQuery = $this->db->query(sprintf($sqlString,$table,$id,$subString));
			//echo sprintf($sqlString,$table,$id);
			$itemSqlString = "SELECT itemID FROM %s WHERE warehouseID='%s'";
			/*類別設定，需確認是否僅顯示類別為3=>tools & Equipment的資料
			$sqlSetTYpe = " AND type='3'";	
			$sqlString = $sqlString.$sqlSetType;
			$itemSqlString = $itemSqlString.$sqlSetType;
			*/
			switch($type){
				case '4':
					$subString = " AND lost='1'";
				break;
				case '3':
					$subString = sprintf(" AND itemID IN (SELECT itemID FROM handleDetail WHERE itemID IN(%s) AND itemHandleID IN(SELECT itemHandleID FROM itemHandle WHERE type='3'))",sprintf($itemSqlString,$table,$id));
				break;
				case '2':
					$subString = sprintf(" AND itemID IN (SELECT itemID FROM handleDetail WHERE itemID IN(%s) AND itemHandleID IN(SELECT itemHandleID FROM itemHandle WHERE type='2'))",sprintf($itemSqlString,$table,$id));
				break;
				case '1':
					$subString = sprintf(" AND itemID IN (SELECT itemID FROM handleDetail WHERE itemID IN(%s) AND itemHandleID IN(SELECT itemHandleID FROM itemHandle WHERE type='1'))",sprintf($itemSqlString,$table,$id));
				break;
				case '0':
				default:
					$subString = "";
			...
			//echo sprintf($sqlString,$table,$id)."<br />";
			$infoData['tableName'] = $table;
		...
		public function returnGridData($infoData){
			...
			foreach($query->result() as $row){
				if($tableName == 'warehouse'){
					$employee = $this->getWareManagerData($row->warehouseManagerID);
					//var_dump($employee);
				}				
				$menuTypeData = array();
		...
		//取得倉庫管理員資料
		private function getWareManagerData($managerID){
			$sqlString = sprintf("SELECT * FROM employee WHERE employeeID='%s'",$managerID);
			$employeeFields = $this->db->list_fields('employee');
			//var_dump($employeeFields);
			$query = $this->db->query($sqlString);
			$result = $query->row();
			$managerData = array();
			foreach($employeeFields as $val){
				$managerData[$val] = isset($result->$val)?$result->$val:"";
			}
			return $managerData;
		}
		...
		public function getFunctionData($tableName=""){
			...
			if($table == 'warehouse'){
				if(!isset($result['managerImg']) || $result['managerImg'] == ''){
