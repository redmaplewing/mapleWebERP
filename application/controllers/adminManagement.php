<?php
	...
	class AdminManagement extends MY_Controller {	
		...
		public function sendNewsData($type='1'){
		/*
		$type-員工所在地
		...
		*/
		...
		$tableID = $table."ID";		
		$sqlString = "SELECT * FROM %s WHERE type='%s' ORDER BY %s ASC";
			...
			foreach($query->result() as $row){
				...
				foreach($infoData['column'] as $val){
					switch($val){
						...
						break;
						case 'file':
							$data = '<a href="'.base_url().'upload/'.$row->$val.'" target="_blank">'.$row->$val.'</a>';
						break;
						default:
							$data = $row->$val;
						break;
		...
		//查詢項目資料並回傳
		public function getNewsData($type='1'){
			$table = "news";
			$tableID = $table."ID";
			$sqlString = "SELECT * FROM %s WHERE type='%s' AND %s='%s' ";
			$sqlString = sprintf($sqlString,$table,$type,$tableID,$this->input->post('ID'));
