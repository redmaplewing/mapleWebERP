...
class RecruProcess extends MY_Controller {
    ...
    public function sendEmployeeData($type = '1') {
        ...
        //echo $subSqlString."<br />";
        //echo sprintf($sqlString,$table,$subSqlString,$tableID)."<br />";
            ...
            foreach ($infoData['column'] as $val) {
                switch ($val) {
                    ...
                    case "ga":
                        ...
                        break;
                    case 'employeeNo':
                        $data = $this->employeeUnUse[$row->employeeID]['No'];
                        break;
                    case "name":
                    ...
                    case "Gender":
                        ...
                        break;
                    case 'requesterID':
                        $data = $this->employeeUnUse[$row->employeeID]['name'];
                        break;
                    case "interviewer1st":
                    case "interviewer2nd":
                        $data = '';
                        if ($row->$val != 0) {
                            if (isset($this->employee[$row->$val]['name'])) {
                                $data = $this->employee[$row->$val]['name'];
                            } else {
                                $data = 'employee was delete';
                            }
                        } else {
                            $data = "";
                        }
                        //$data = $row->$val != 0 ? $this->employee[$row->$val]['name'] : "";
                        break;
                    case "pass":
                    case "resultConfirm":
    ...
    private function _load() {
        ...
        $sqlString2 = "SELECT * FROM employee";
        ...
        $dbQuery = $this->db->query($sqlString2);
        $employeeUnUse = array();
        foreach ($dbQuery->result() as $key => $val) {
            $employeeUnUse[$val->employeeID]['name'] = $val->nameFirst . $val->nameLast;
            $employeeUnUse[$val->employeeID]['No'] = $val->emplyoeeNo;
            $employeeUnUse[$val->employeeID]['groupID'] = $val->groupID;
        };
        //var_dump($employee);
        $this->tableName = $tableName;
        $this->employee = $employee;
        $this->employeeUnUse = $employeeUnUse;
    }
