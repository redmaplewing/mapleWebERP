$(function(){
	...
	function resetTEData(ids){
		...
		$("#repairTable").jqGrid().setGridParam({url : base_url+'/getInventoryTool/'+ids+'/1'});		
		$("#calibrationTable").jqGrid().setGridParam({url : base_url+'/getInventoryTool/'+ids+'/2'});		
		$("#disposedTable").jqGrid().setGridParam({url : base_url+'/getInventoryTool/'+ids+'/3'});
		$("#lostTable").jqGrid().setGridParam({url : base_url+'/getInventoryTool/'+ids+'/4'});
		$("#workingTable").trigger("reloadGrid");
	...
	function showFormDetail(type,ids,tar){
		...
		switch(type){				
			case 'edit':				
				...
				if(tar.attr('id') == dialogDiv.attr('id')){
					resetTEData(ids);
				}				
